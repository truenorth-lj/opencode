  ...
  it.live(
    ...
    withTmp({ git: true, config: { formatter: false, lsp: false } }, (tmp) =>
      Effect.gen(function* () {
        ...
        expect(
          yield* requestJson<SessionMessage.Message[]>(
            pathFor(SessionPaths.messages, { sessionID: parent.id }),
            { headers },
          ),
        ).toEqual(expect.arrayContaining([expect.objectContaining({ type: "assistant" })]))
      }),
    ),
  )
